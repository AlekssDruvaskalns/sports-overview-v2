<x-app-layout>
    ...
    <div class="flex justify-end mb-4">
        <a href="{{ route('event.create') }}" class="button">
            <div class="button-overlay"></div>
            <span>Create Event <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 53 58" height="58" width="53">
    ...
    <div class="gap-6">
        ...
        <div class="w-full rounded overflow-hidden shadow-lg p-4 bg-white mb-4">
            <div class="flex justify-between">
                <div>       
                    <a class=" hover:drop-shadow transform hover:bg-gray-100 font-bold text-xl mb-2" href="{{ route('event.show', $event->id) }}">
                        {{ $event->name }}
                    </a>     
                ...
                <div>
                    <a href="{{ route('event.show', $event->id) }}" class="bg-blue-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">
                        View
                    </a>
                    <a href="{{ route('event.edit', $event->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">
                        Edit
                    </a>
                    <a href="{{ route('event.destroy', $event->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">
                        Delete
                    </a>
                </div>
            </div>
